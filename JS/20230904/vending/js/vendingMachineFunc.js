    ...
    bindEvents() {
        ...
        this.btnPut.addEventListener('click', () => {
            ...
            if (inputCost) {
                if (inputCost <= myMoneyVal && inputCost > 0) {
                    // Intl 객체를 통해서 사용자 국가에서 사용하는 숫자 포맷으로 변환한다.
                    this.myMoney.textContent = new Intl.NumberFormat().format
                        (myMoneyVal - inputCost) + '원';
        ...
        colaBtns.forEach((colaBtn) => {
            colaBtn.addEventListener('click', (event) => {
                ...
                let isStaged = false; // 선택한 콜라가 이미 장바구니에 들어가 있는지 확인하기 위한 변수

                // 잔액이 콜라 가격보다 많거나 동일하다면 
                if (balanceVal >= targetElPrice) {
                    this.balance.textContent = new Intl.NumberFormat().format(balanceVal - targetElPrice) + '원';

                    // 장바구니에 들어있는 이름과 사용자가 클릭한 이름이 동일하면 수량 증가
                    // forEach를 사용하면 모든 요소를 다 돌기 때문에 비효율적
                    ...
                    // 콜라를 장바구니에 담기
                    // 만약 처음 선택한 콜라라면 장바구니에 담기 
                    if (!isStaged) {
                        this.stagedItemGenerator(colaBtn); // stagedItemGenerator는 클래스 안에 있기 때문에 this를 붙여줘야 한다.
                    }
